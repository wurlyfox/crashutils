...
const char* g_szClassName = "PropGridCtl"; ///< The classname.
...
typedef struct tagLISTBOXITEM {
    char* lpszCatalog; ///< Catalog (group) name
    char* lpszPropName;///< Property (item) name
    char* lpszMisc;    ///< Item specific data string
    char* lpszPropDesc;///< Property (item) description
    char* lpszCurValue;///< Property (item) value
...
{
    *ppInstanceData = (LPINSTANCEDATA)GetProp(hControl, (const char*)_T("lpInsData"));
...
static BOOL Control_CreateInstanceData(HWND hControl, LPINSTANCEDATA pInstanceData)
{
    ...
    return SetProp(hControl, (const char*)_T("lpInsData"), pInst);
}
...
static BOOL Control_FreeInstanceData(HWND hControl)
{
    ...
    if (Control_GetInstanceData(hControl, &pInst))
    {
        ...
        RemoveProp(hControl, (const char*)_T("lpInsData"));
        return TRUE;
    }
    return FALSE;
}
...
static char* NewString(char* str)
{
    ...
    char* tmp = (char*)calloc(strlen((const char*)str) + 1, sizeof(TCHAR));

    if(NULL == tmp)
    {
        return (char*)calloc(1, sizeof(TCHAR)); 
    }
    return (char*)memmove(tmp, str, strlen((const char*)str));
}
...
static char* NewStringArray(char* szzStr)
{
    ...
    for (char* p = szzStr; *p; (p += strlen((const char*)p) + 1, iLen = p - szzStr)) ;

    //Allocate for array
    char* tmp = (char*)calloc(iLen + 1, sizeof(TCHAR));

    if(NULL == tmp)
    {
        return (char*)calloc(1, sizeof(TCHAR)); 
    }
    return (char*)memmove(tmp, szzStr, iLen);
}
...
static LPLISTBOXITEM NewItem(char* szCatalog, char* szPropName, char* szCurValue, char* szMisc, char* szPropDesc, INT iItemType)
{
...
static COLORREF GetColor(char* src)
{
    INT RVal, GVal, BVal;
    RVal = GVal = BVal = 0;
    if (strlen((const char*)src) > 0)
        scanf((const char*)src, _T("%d,%d,%d"), &RVal, &GVal, &BVal);
...
static LRESULT CALLBACK IpEdit_Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    BOOL fEdit = (0 == strcmp((const char*)buf, (const char*)WC_EDIT));
    ...
    else if (WM_CHAR == msg && VK_RETURN == wParam)
    {
        if (NULL != g_lpInst->lpCurrent)
        {
            ...
            {
                sprintf((char*)buf, "%d.%d.%d.%d", ip[3], ip[2], ip[1], ip[0]);
                AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, buf);
            }
        }
        ShowWindow(hIpEdit, SW_HIDE);
        ...
        if (NULL != g_lpInst->lpCurrent)
        {
            switch (g_lpInst->lpCurrent->iItemType)
            {
                ...
                case PIT_DATETIME:
                {
                    ...
                    GetTimeFormat(LOCALE_USER_DEFAULT, 0, &st, _T("hh':'mm':'ss tt"), (char*) (buf + strlen((const char*)(const char*)buf)), MAX_PATH - strlen((const char*)(const char*)buf));
                    AllocatedString_Replace(g_lpInst->lpCurrent->lpszCurValue, buf);
                    ShowWindow(g_lpInst->hwndCtl1, SW_HIDE);
                    ShowWindow(g_lpInst->hwndCtl2, SW_HIDE);
                }
...
///       behavior to capture and subclass the child edit control.  This edit control
...
static LRESULT CALLBACK ComboBox_Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    BOOL fEdit = (0 == strcmp((const char*)classname, (const char*)WC_EDIT));
    ...
    else if (WM_COMMAND == msg)
    {
        ...
        HWND hwndCtl = GET_WM_COMMAND_HWND(wParam, lParam);
        {
            ...
            if (NULL == lpfn)
            {
                ...
                if (0 == strcmp((const char*)classname, (const char*)WC_EDIT))
                {
...
///
/// @returns VOID.
static VOID LogFontItem_FromString(LPPROPGRIDFONTITEM lpLogFontItem, char* lpszFont)
{
    LPLOGFONT lpLf = (LPLOGFONT)lpLogFontItem;
    scanf((const char*)lpszFont,
        _T( "Height: %d " \
            "Width: %d " \
            "Escapement: %d " \
            "Orientation:  %d " \
...
///
/// @returns char* The string representation of PROPGRIDFONTITEM elements.
static char* LogFontItem_ToString(LPPROPGRIDFONTITEM lpLogFontItem)
{
    static TCHAR buf[MAX_PATH];
    memset(buf, (TCHAR)0, MAX_PATH);
...
///
/// @returns VOID.
static VOID FileDialogItem_FromString(LPPROPGRIDFDITEM lpPgFdItem, char* lpszFdItem)
{
    //DWM 1.2: Added method
    static TCHAR PgFdItem[4][MAX_PATH];
    memset(PgFdItem, (TCHAR)0, sizeof(PgFdItem));

    scanf((const char*)lpszFdItem,
#ifdef _UNICODE
        _T( "Title: %256l[^\r\n] " \
            "Path: %256l[^\r\n] " \
            "Filter: %256l[^\r\n] " \
    ...
    {
        if (0 == strcmp((const char*)_T("?"), (const char*)PgFdItem[i]))
        {
        ...
        else if(2 == i)
        {
            ...
            for (char* ptr = PgFdItem[2]; *ptr; ptr++)
                if (_T('\t') == *ptr) *ptr = _T('\0');
        }
    }
...
///
/// @returns char* The string representation of PROPGRIDFDITEM elements.
static char* FileDialogItem_ToString(LPPROPGRIDFDITEM lpPgFdItem)
{
    static TCHAR szBuf[3 * MAX_PATH];
    memset(szBuf, (TCHAR)0, NELEMS(szBuf));
    ...
    INT iLen = 0;
    for (char* psz = (char*)lpPgFdItem->lpszFilter,
          ps = filter, pe = filter + NELEMS(filter) - 1;
           *psz && ps < pe; psz += iLen + 1)
    {
        memmove(ps, psz, (iLen = strlen((const char*)psz)));
        ps += iLen;
        *ps++ = _T('\t');
    }
  EXIT_FOR:
    ...
    sprintf((char*)szBuf, 
         ...
         "Title: %ls\r\n" \
            ...
            "Default Extension: %3s",
#endif
        0 < strlen((const char*)lpPgFdItem->lpszDlgTitle) ? lpPgFdItem->lpszDlgTitle : _T("?"), //DWM 1.2: Added default "?"
        0 < strlen((const char*)lpPgFdItem->lpszFilePath) ? lpPgFdItem->lpszFilePath : _T("?"), //DWM 1.2: Added default "?"
        0 < strlen((const char*)filter) ? filter : _T("?"), //DWM 1.2: Added default "?"
        0 < strlen((const char*)lpPgFdItem->lpszDefExt) ? lpPgFdItem->lpszDefExt : _T("?")); //DWM 1.2: Added default "?"
        */
    return szBuf;
}
...
static char* BrowseFolder(HWND hwnd, char* curPath, char* title, char* rootPath)
{
    BROWSEINFO bi;
...
static INT ListBox_FindCatalog(HWND hwnd, INT indexStart, const char* szCatalog)
{
    INT ln = strlen((const char*)szCatalog) + 1;
    ...
    for (INT i = indexStart < 0 ? 0 : indexStart; i < ListBox_GetCount(hwnd); i++)
    {
        ...
        if (NULL != pItem && 0 == strcmp((const char*)pItem->lpszCatalog, (const char*)szCatalog))
            return i;
    }
    return LB_ERR;
}
...
static VOID ToggleCatalog(LPLISTBOXITEM pItem)
{
    ...
    if (pItem->fCollapsed)
    {
        for (INT i = ListBox_GetCount(g_lpInst->hwndListBox) - 1; i >= 0; i--)
        {
            ...
            if (NULL != p && strcmp((const char*)p->lpszCatalog, (const char*)pItem->lpszCatalog) == 0)
            {
    ...
    else
    {
        ...
        if (LB_ERR != idx)
        {
            ...
            for (INT i = ListBox_GetCount(g_lpInst->hwndListMap) - 1; 0 <= i; i--)
            {
                ...
                {
                    if (0 == strcmp((const char*)pp->lpszCatalog, (const char*)pItem->lpszCatalog))
                    {
    ...
    if (NULL != g_lpInst->lpCurrent)
    {
        switch(g_lpInst->lpCurrent->iItemType)//DWM 1.3: Added switch
        {
            ...
            case PIT_STATIC:
                return;
            case PIT_CHECK:
                if(0 == strcmp((const char*)g_lpInst->lpCurrent->lpszMisc, (const char*)SELECT))
                {
            ...
            if (NULL != pItem)
            {
                switch (pItem->iItemType)
                {
                    case PIT_FONT:
                    {
                        ...
                        sprintf((char*)buf, (const char*)szFmt, pgfi.logFont.lfFaceName, PointSize);
                    }
                        break;
                    ...
                    default:
                        //Replace the text in the buf and update the tool
                        strncat((char*)buf, (const char*)pItem->lpszCurValue, NELEMS(buf) - 1);
                        break;
                }
            }
        }
...
static VOID ListBox_OnSelectIP(HWND hwnd, RECT rc, LPLISTBOXITEM pItem)
{
    ...
    BYTE ip[4] = { 0, 0, 0, 0 };
    scanf((const char*)pItem->lpszCurValue, "%hhd.%hhd.%hhd.%hhd", &ip[3], &ip[2], &ip[1], &ip[0]);
...
static VOID ListBox_OnSelectDateTime(HWND hwnd, RECT rc, LPLISTBOXITEM pItem)
{
    ...
    if (PIT_DATE == pItem->iItemType)
    {
        ...
        if (NULL != pItem)
            scanf((const char*)pItem->lpszCurValue, _T("%hd/%hd/%hd"), &st.wMonth, &st.wDay, &st.wYear);
    ...
    else if (PIT_TIME == pItem->iItemType)
    {
        ...
        if (NULL != pItem)
            scanf((const char*)pItem->lpszCurValue, szFormat, &st.wHour, &st.wMinute, &st.wSecond, &buf);

        if (0 == strcmp((const char*)_T("PM"), (const char*)buf))
            st.wHour += 12;
    ...
    else if (PIT_DATETIME == pItem->iItemType)
    {
        ...
        if (NULL != pItem)
            scanf((const char*)pItem->lpszCurValue, szFormat, &st.wMonth, &st.wDay, &st.wYear, &st.wHour, &st.wMinute, &st.wSecond, &buf);
...
static VOID ListBox_OnSelectComboBox(HWND hwnd, RECT rc, LPLISTBOXITEM pItem)
{
    ...
    for (char* p = pItem->lpszMisc; *p; p += strlen((const char*)p) + 1)
    {
...
static VOID ListBox_OnKeyDown(HWND hwnd, UINT vk, BOOL fDown, INT cRepeat, UINT flags)
{
    ...
    if (NULL != g_lpInst->lpCurrent)
    {
        ...
        if (PIT_CHECK == g_lpInst->lpCurrent->iItemType && (VK_SPACE == vk || VK_RETURN == vk))
        {
            if (0 == strcmp((const char*)g_lpInst->lpCurrent->lpszMisc, (const char*)SELECT))
            {
                g_lpInst->lpCurrent->lpszCurValue = (char*)(0 == strcmp((const char*)g_lpInst->lpCurrent->lpszCurValue, (const char*)CHECKED) ? UNCHECKED : CHECKED);
        ...
        else if (PIT_CHECK == g_lpInst->lpCurrent->iItemType && VK_ESCAPE == vk)
        {
            ...
            RedrawWindow(hwnd, &rc, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_UPDATENOW);
                ...
                0 == strcmp((const char*)g_lpInst->lpCurrent->lpszMisc, (const char*)SELECT))
        {
            fHandled = TRUE;
        }
        else
            ...
            if (VK_TAB == vk)
            {
                ...
                switch (g_lpInst->lpCurrent->iItemType)
                {
                    ...
                    case PIT_CHECK:
                        ...
                        else if(0 == strcmp((const char*)g_lpInst->lpCurrent->lpszMisc, (const char*)SELECT))
                        {
                            g_lpInst->lpCurrent->lpszMisc = UNSELECT;
                            SetFocusToParent();
                        }
    ...
    if (ID_BUTTON == id)
    {
        ...
        switch (g_lpInst->lpCurrent->iItemType)
        {
            case PIT_COLOR:
            {
                ...
                {
                    TCHAR buf[MAX_PATH];
                    sprintf((char*)buf, "%d,%d,%d", GetRValue(cc.rgbResult), GetGValue(cc.rgbResult), GetBValue(cc.rgbResult));
            ...
            case PIT_FILE:
            {
                ...
                sscanf((const char*)g_lpInst->lpCurrent->lpszMisc,
#ifdef _UNICODE
                     "Title: %32l[^\r\n] " \
                        "Path: %256l[^\r\n] " \
                        "Filter: %256l[^\r\n] " \
                ...
                if (0 != strcmp((const char*)_T("?"), (const char*)title)) //DWM 1.2: Added test
                    ofn.lpstrTitle = title;
                else
                    ofn.lpstrTitle = _T("Select file");

                if (0 != strcmp((const char*)_T("?"), (const char*)path)) //DWM 1.2: Added test
                {
                    //Exclude the filename
                    for (char* ptr = path + strlen((const char*)path) - 1; *ptr; ptr--)
                        if (*ptr == _T('\\'))
                        {
                            *ptr = _T('\0');
                            break;
                ...
                if (0 != strcmp((const char*)_T("?"), (const char*)filter)) //DWM 1.2: Added test
                {
                    //Convert back to double null-terminated string
                    for (char* ptr = filter; *ptr; ptr++)
                        if (_T('\t') == *ptr)
                            *ptr = _T('\0');
                ...
                if (0 != strcmp((const char*)_T("?"), (const char*)ext)) //DWM 1.2: Added test
                {
                    ofn.lpstrDefExt = ext;
                }
                else
                ...
                {
                    PROPGRIDFDITEM pgi;
                    pgi.lpszDlgTitle = (char*)ofn.lpstrTitle;
                    pgi.lpszFilePath = (char*)ofn.lpstrFile;
                    pgi.lpszFilter = (char*)ofn.lpstrFilter;
                    pgi.lpszDefExt = (char*)ofn.lpstrDefExt;
            ...
            case PIT_FOLDER:
            {
                char* temp = BrowseFolder(hwnd, g_lpInst->lpCurrent->lpszCurValue, _T(""), _T(""));
                //DWM 1.2: Reset to unselected folder //if (0 < strlen((const char*)temp))
...
static HBRUSH Grid_OnCtlColorStatic(HWND hwnd, HDC hdc, HWND hwndChild, INT type)
{
    ...
    if (NULL != g_lpInst->hwndPropDesc)
    {
        ...
        HDC hdc = GetDC(hwnd);
        RECT r = MAKE_RECT(0, g_lpInst->iVDivider - 2,
            WIDTH(rc), g_lpInst->iVDivider);
...
///
/// @returns VOID.
static VOID Grid_ExpandCatalog(const char* szCatalog)
{
    if (NULL != szCatalog)
    {
        INT ln = strlen((const char*)szCatalog) + 1; //Check for empty string
        if (ln == 1)
            return;
    }
    ...
    for (INT i = 0; i < ListBox_GetCount(g_lpInst->hwndListBox); i++)
    {
        ...
        if (NULL != pItem && PIT_CATALOG == pItem->iItemType &&
            (NULL == szCatalog || 0 == strcmp((const char*)pItem->lpszCatalog, (const char*)szCatalog)))
        {
            if (pItem->fCollapsed)
                ToggleCatalog(pItem);
        }
...
///
/// @returns VOID.
static VOID Grid_CollapseCatalog(const char* szCatalog)
{
    if (NULL != szCatalog)
    {
        INT ln = strlen((const char*)szCatalog) + 1; //Check for empty string
                ...
                if (NULL != pItem)
                {
                    if (PIT_CHECK == pItem->iItemType)
                    {
                        if (iCurSel != g_lpInst->iPrevSel &&
                            0 == strcmp((const char*)pItem->lpszMisc, (const char*)SELECT))
...
static VOID Grid_OnDrawItem(HWND hwnd, const DRAWITEMSTRUCT *lpDIS)
{
    ...
    if (PIT_CATALOG == pItem->iItemType)
    {
        ...
        RECT r = MAKE_RECT(rectCatPart1.left + 6, rectCatPart1.top + 3,
            rectCatPart1.right - 6, rectCatPart1.bottom + 3);
        DrawText(lpDIS->hDC, pItem->lpszCatalog, strlen((const char*)pItem->lpszCatalog),
            &r,
    ...
    else
    {
        ...
        RECT r = MAKE_RECT(rectPart1.left + 3, rectPart1.top + 3, rectPart1.right - 3,
            rectPart1.bottom + 3);
        DrawText(lpDIS->hDC, pItem->lpszPropName, strlen((const char*)pItem->lpszPropName),
            &r, DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
    ...
    if (PIT_CATALOG != pItem->iItemType)
    {
        ...
        {
            if (0 == strcmp((const char*)pItem->lpszMisc, (const char*)SELECT))
            {
                FillRect(lpDIS->hDC, &rectPart2,
                    GetSysColorBrush(COLOR_HIGHLIGHT));
            }
            DrawFrameControl(lpDIS->hDC, &rect3, DFC_BUTTON, DFCS_BUTTONCHECK |
                (strcmp((const char*)pItem->lpszCurValue, (const char*)CHECKED) == 0 ? DFCS_CHECKED : 0));
    ...
    if (PIT_CHECK != pItem->iItemType)
    {
...
#endif
                sprintf((char*)buf, (const char*)szFmt, pgfi.logFont.lfFaceName, PointSize);
                RECT r = MAKE_RECT(rectPart3.left + 3, rectPart3.top + 3,
                    rectPart3.right + 3, rectPart3.bottom + 3);
                DrawText(lpDIS->hDC, buf, strlen((const char*)buf),
                    &r,
                    DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
        ...
        else
        {
            ...
            RECT r = MAKE_RECT(rectPart3.left + 3, rectPart3.top + 3,
                rectPart3.right + 3, rectPart3.bottom + 3);
            DrawText(lpDIS->hDC, pItem->lpszCurValue, strlen((const char*)pItem->lpszCurValue),
                &r,
                DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
        }
    }
...
static INT Grid_OnAddString(LPPROPGRIDITEM pgi)
{
    ...
    switch (pgi->iItemType)
    {
        ...
        case PIT_EDITCOMBO:
        case PIT_FOLDER:
            lpszCurValue = (char*)pgi->lpCurValue;
            break;
        case PIT_COLOR:
        ...
        case PIT_FONT:
            ...
            break;
        case PIT_CHECK:
            lpszCurValue = (BOOL)pgi->lpCurValue ? (char*)CHECKED : (char*)UNCHECKED;
            break;
        case PIT_FILE:
        ...
        case PIT_DATETIME:
            ...
            strcat((char*)buf, " ");
            GetTimeFormat(LOCALE_USER_DEFAULT, 0, (LPSYSTEMTIME)pgi->lpCurValue,
                _T("hh':'mm':'ss tt"), (char*) (buf + strlen((const char*)buf)),
                MAX_PATH - strlen((const char*)buf));
            lpszCurValue = buf;
            break;
    }
        ...
        if (PIT_CATALOG != pItem->iItemType)
        {
            ...
            switch (pgi.iItemType)
            {
                ...
                case PIT_FONT:
                    ...
                    break;
                case PIT_CHECK:
                    pgi.lpCurValue = (LPARAM) (0 == strcmp((const char*)CHECKED,
                        (const char*)pItem->lpszCurValue));
                    break;
                case PIT_FILE:
                ...
                {
                    BYTE ip[4] = { 0, 0, 0, 0 };
                    scanf((const char*)pItem->lpszCurValue, _T("%hhd.%hhd.%hhd.%hhd"),
                        &ip[3], &ip[2], &ip[1], &ip[0]);
                ...
                case PIT_DATE:
                    memset(&st, 0, sizeof(SYSTEMTIME));
                    scanf((const char*)pItem->lpszCurValue, _T("%hd/%hd/%hd"),
                        &st.wMonth, &st.wDay, &st.wYear);
                    pgi.lpCurValue = (LPARAM) & st;
                    break;
                case PIT_TIME:
                    memset(&st, 0, sizeof(SYSTEMTIME));
                    memset(outbuf, (TCHAR)0, MAX_PATH);
                    scanf((const char*)pItem->lpszCurValue,
#ifdef _UNICODE
                        _T("%hd:%hd:%hd %2ls"),
#else
                        _T("%hd:%hd:%hd %2s"),
                    ...
                    if (0 == strcmp((const char*)_T("PM"), (const char*)outbuf))
                        st.wHour += 12;
...
static LRESULT CALLBACK Grid_Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        ...
        {
            if (NULL == (char*)lParam) //Expand all
            {
            ...
            else
            {
                ...
                for (char* p = (char*)lParam; *p; p += strlen((const char*)p) + 1)
                    Grid_ExpandCatalog((const char*)p);
            }
        }
            break;
        case PG_COLLAPSECATALOGS:
        {
            if (NULL == (char*)lParam) //Collapse all
            {
            ...
            {
                for (char* p = (char*)lParam; *p; p += strlen((const char*)p) + 1)
                    Grid_CollapseCatalog((const char*)p);
            }
        }
            break;
